...
public class PromotionService {
    ...
    public Page<Promotion> findAll(String keyword, Pageable pageable) {
        ...
        if (keyword != null && !keyword.isEmpty()) {
            // Bạn cần viết thêm query trong PromotionRepository: findByNameContainingOrCodeContaining
    ...
    // (Danh mục thường ít bị lỗi Proxy hơn vì nó đơn giản, nhưng nếu bị thì làm tương tự)

    @Transactional(readOnly = true)
    public Set<Category> getCategoriesByPromotion(Long promoId) {
        Promotion p = findById(promoId);
        // Ép tải dữ liệu (nếu cần thiết để tránh LazyInitializationException bên Controller)
