...
-- ============================================
-- BẢNG QUẢN LÝ NHÀ CUNG CẤP VÀ NHẬP KHO
-- ============================================

-- 20. Bảng `suppliers` (Nhà cung cấp)
CREATE TABLE `suppliers` (
    `id` BIGINT NOT NULL AUTO_INCREMENT,
    `name` VARCHAR(255) NOT NULL,                 -- Tên công ty hoặc nhà cung cấp
    `contact_person` VARCHAR(255) NULL,            -- Tên người liên hệ
...
-- 21. Bảng `goods_receipts` (Phiếu nhập kho)
CREATE TABLE `goods_receipts` (
    `id` BIGINT NOT NULL AUTO_INCREMENT,
    `supplier_id` BIGINT NULL,
    `receipt_code` VARCHAR(50) UNIQUE NOT NULL,    -- Mã phiếu nhập tự động tạo (VD: PN20231027001)
    `note` TEXT NULL,
    `creator_id` BIGINT NOT NULL,                  -- ID của người dùng (admin) tạo phiếu
    `total_amount` DECIMAL(15, 2) NOT NULL DEFAULT 0.00, -- Tổng giá trị của phiếu nhập
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
-- 22. Bảng `goods_receipt_items` (Chi tiết phiếu nhập)
CREATE TABLE `goods_receipt_items` (
    `id` BIGINT NOT NULL AUTO_INCREMENT,
    `receipt_id` BIGINT NOT NULL,
    `variant_id` BIGINT NOT NULL,
    `quantity` INT NOT NULL,                          -- Số lượng nhập
    `import_price` DECIMAL(10, 2) NOT NULL,           -- Giá nhập tại thời điểm này
...
BEGIN
    ...
    IF NEW.quantity < 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot insert goods_receipt_item with negative quantity';
    END IF;

    -- Cộng số lượng vào stock và cập nhật giá/cost
    UPDATE product_variants
    SET stock_quantity = stock_quantity + NEW.quantity,
        cost_price = NEW.import_price,
        price = CASE WHEN NEW.new_selling_price IS NOT NULL THEN NEW.new_selling_price ELSE price END
...
-- Trigger: sau khi cập nhật một mục nhập (goods_receipt_items), điều chỉnh tồn kho và giá
DELIMITER //

CREATE TRIGGER trg_update_variant_after_goods_receipt_item_update
...
BEGIN
    ...
    -- Không cho phép cập nhật thành số lượng âm
    IF NEW.quantity < 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot update goods_receipt_item to negative quantity';
    END IF;

    -- Nếu variant thay đổi, kiểm tra tồn của variant cũ trước khi trừ
    IF OLD.variant_id <> NEW.variant_id THEN
        -- Kiểm tra tồn của variant cũ
        SELECT stock_quantity INTO current_stock FROM product_variants WHERE variant_id = OLD.variant_id;
        IF current_stock - OLD.quantity < 0 THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Operation would cause negative stock on old variant';
        END IF;

        -- Trừ tồn cho variant cũ
        UPDATE product_variants
        SET stock_quantity = stock_quantity - OLD.quantity
        WHERE variant_id = OLD.variant_id;

        -- Cộng tồn cho variant mới và cập nhật giá
        UPDATE product_variants
        SET stock_quantity = stock_quantity + NEW.quantity,
            cost_price = NEW.import_price,
            price = CASE WHEN NEW.new_selling_price IS NOT NULL THEN NEW.new_selling_price ELSE price END
        WHERE variant_id = NEW.variant_id;
    ELSE
        -- variant không thay đổi: điều chỉnh theo chênh lệch số lượng
        SET qty_diff = NEW.quantity - OLD.quantity;

        SELECT stock_quantity INTO current_stock FROM product_variants WHERE variant_id = NEW.variant_id;
        IF current_stock + qty_diff < 0 THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Operation would cause negative stock on variant';
        END IF;
...
-- Trigger: sau khi xóa một mục nhập (goods_receipt_items), trừ tồn tương ứng
DELIMITER //

CREATE TRIGGER trg_update_variant_after_goods_receipt_item_delete
...
BEGIN
    ...
    SELECT stock_quantity INTO current_stock FROM product_variants WHERE variant_id = OLD.variant_id;
    IF current_stock - OLD.quantity < 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Operation would cause negative stock on variant (delete)';
    END IF;
